...
function cargaEquipos(){
	...
	$.getJSON("php/equiposActivos.php", function(data){
		...
		for (var i = 0; i < unicamarca.length; i++) {
			...
			$("#brandconten").append("<li><a class='categories' id='."+unicamarca[i].toLowerCase()+"' onclick='filtraequipos(this.id)'>"+unicamarca[i]+"</a></li>");
		}
		
		$("#brandconten").append("<li><a class='categories' id='todos' onclick='filtraequipos(this.id)'>Todos</a></li>");
